...
static long process_step(int pnum, Process *q) {
   ...
   if (q->pages[page]!=0) {
	if (!q->blocked[page]) {
	    sim_log(LOG_BLOCK,"pc=%d process=%2d page=%3d blocked\n",processes[pnum]->pc,pnum,page);
	...
	if (q->blocked[page]) {
	    sim_log(LOG_BLOCK,"pc=%d process=%2d page=%3d unblocked\n",processes[pnum]->pc,pnum,page);
...
sim_log(LOG_PAGE,"pc=%d process=%2d page=%3d start pageout\n",processes[process]->pc,process,page);
...
int pagein(int process, int page) {
    ...
    sim_log(LOG_PAGE,"pc=%d process=%2d page=%3d start pagein\n",processes[process]->pc,process,page);
...
static void initqueue() {
   ...
   for (i=0; i<QUEUESIZE; i++) {
        ...
        // queue + i == queue[i] and programs + queuetype[i] == programs[queuetype[i]]
...
static void allage () {
   ...
   for (i=0; i<procs; i++) {
	   ...
	   for (j=0; j<processes[i]->npages; j++) {
		    ...
		    processes[i]->pages[j]--;
		    if (processes[i]->pages[j]==0) {
			sim_log(LOG_PAGE,"pc=%d process=%2d page=%3d end   pagein\n",processes[i]->pc,i,j);
			...
			sim_log(LOG_PAGE,"pc=%d process=%2d page=%3d end   pageout\n",processes[i]->pc,i,j);
